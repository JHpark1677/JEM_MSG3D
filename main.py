...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if arg.phase == 'train':
            ...
            arg.train_feeder_args['debug'] = arg.train_feeder_args['debug'] or self.arg.debug # the arguments of data loader for training (default : dict())
            logdir = os.path.join(arg.work_dir, 'trainlogs') # joint various path components (work_dir/trainlogs path를 얻는다.)
            if not arg.train_feeder_args['debug']: # train-log already exists
    ...
    def train(self, epoch, save_model=False):
        ...
        # data_loader['train'] initialized with torch.utils.data.DataLoader
        # dataset from Feeder, batch_size, shuffle, num_workers, drop_last, and worker_init_fn
        # feeder all knows data path or ~
        loss_values = []
        ...
        for batch_idx, (data, label, index) in enumerate(process): # training iteration 
            self.global_step += 1
            # get data
            with torch.no_grad():
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        with torch.no_grad():
            ...
            self.model.eval()
            self.print_log(f'Eval epoch: {epoch + 1}') # eval epoch = 1
            for ln in loader_name: # only for the loader_name = test
                loss_values = []
                score_batches = []
                step = 0
                ...
                # self.data_loader['test] = torch.utils.data.DataLoader(
                # dataset = Feeder ~, batch_size = self.arg.batch_size, shuffle, num_workers, drop_last, worker_init_fn ~)
                for batch_idx, (data, label, index) in enumerate(process): # interation number 1238
                    data = data.float().cuda(self.output_device) 
                    label = label.long().cuda(self.output_device) # get data and label from output_device
                    output = self.model(data)
                    if isinstance(output, tuple):
                        output, l1 = output # if output is tuple split it into two
                        l1 = l1.mean()
                    else:
                        l1 = 0
                    loss = self.loss(output, label) # calculate output vs label
                    # loss - nn.CrossEntropyLoss().cuda(output_device) .. it means calculated from cuda
                    score_batches.append(output.data.cpu().numpy())
                    loss_values.append(loss.item()) # collect all loss values

                    _, predict_label = torch.max(output.data, 1) # output.data를 one dimension으로 reduce 한 뒤 max를 얻음
                    step += 1 # increase step number

                    if wrong_file is not None or result_file is not None:
                        predict = list(predict_label.cpu().numpy())
                        true = list(label.data.cpu().numpy()) # true labels and predicted labels
                        for i, x in enumerate(predict):
                            if result_file is not None:
                                f_r.write(str(x) + ',' + str(true[i]) + '\n')
                            if x != true[i] and wrong_file is not None:
                                f_w.write(str(index[i]) + ',' + str(x) + ',' + str(true[i]) + '\n')
                                # result file이 exist하면 다음을 받아적는다.

            score = np.concatenate(score_batches)
            loss = np.mean(loss_values) # loss_values are dictionary of losses for all batches
            accuracy = self.data_loader[ln].dataset.top_k(score, 1) # top1 accuracy
            if accuracy > self.best_acc:
                self.best_acc = accuracy
                self.best_acc_epoch = epoch + 1

            print('Accuracy: ', accuracy, ' model: ', self.arg.work_dir) # work_directory pretrained model
            if self.arg.phase == 'train' and not self.arg.debug:
                self.val_writer.add_scalar('loss', loss, self.global_step)
                self.val_writer.add_scalar('loss_l1', l1, self.global_step)
                self.val_writer.add_scalar('acc', accuracy, self.global_step)

            score_dict = dict(zip(self.data_loader[ln].dataset.sample_name, score)) # score dictionary to memo the score
            self.print_log(f'\tMean {ln} loss of {len(self.data_loader[ln])} batches: {np.mean(loss_values)}.') # for 1238 batches mean loss is
            for k in self.arg.show_topk: # default arg.show_topk = [1,5]
                self.print_log(f'\tTop {k}: {100 * self.data_loader[ln].dataset.top_k(score, k):.2f}%')

            if save_score: # arg to true if you want to save score
...
def main():
    ...
    p = parser.parse_args()
    if p.config is not None: # yaml file exists
        with open(p.config, 'r') as f:
            default_arg = yaml.load(f) # load yaml file
        key = vars(p).keys()
        for k in default_arg.keys(): # not valid if there aren't args' keys of yaml's args
            if k not in key:
                print('WRONG ARG:', k)
                assert (k in key)
        parser.set_defaults(**default_arg)

    arg = parser.parse_args() # insert parse_args to arg variable
    init_seed(arg.seed)
